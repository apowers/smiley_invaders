;       sminv.asm     atom powers     4.28.98 5.21.98 5.5.99
...
MENUTX: DB      CR,LF
	...
	DB      0BAH,'AN ATOMIC RED PRODUCTION    V 5.99   ',0BAH,CR,LF
...
MAIN:   MOV     AX,VIDSEG
	...
	PUSH    DS                      ;not in video seg
	MOV     DS,CS
	MOV     DX,MENUTX               ;display menu
	MOV     AH,09           ;output a string begining in ds:dx
	INT     21H
	POP     DS
	MOV     AH,07           ;wait for keyboard character 
	INT     21H
	CMP     AL,1BH
	JNE     SMINV
	JMP     0
...
SMILE0: MOV     AH,2CH          ;get time
	INT     21H
	MOV     DH,20           ;set shot loops
	ADD     DL,05           ;time slice
	CMP     DL,99           ;fix max time
	JGE     SMILE0
	MOV     W[CLOCK],DX     ;store value
SMILE1: 
	CALL    DOKEY           ;process keystroke
	MOV     AH,2CH          ;get time
	INT     21H
	CMP     DL,B[CLOCK]     ;check time slice
	JL      SMILE1
	ADD     B[CLOCK],05     ;next slice
	DEC     B[CLOCK+1]      ;inc loop
	JNE     SMILE1
	CALL    BMOVE
	CALL    ALMOVE
	CALL    CREATE
...
DOKEY:  ;CMP     AL,0
	;JNE     DOKY0
	MOV     AH,6            ;get key in buffer
	MOV     DX,00FFH        ;set for input
	INT     21H
DOKY0:  CMP     AL,ESC
	JNE     DOKY1
	JMP     MAIN
...
GOVERB: CALL    DRAWIT
	...
	DEC     CH
	JNE     GOVERB
GOVERC: MOV     AH,07           ;wait for keyboard character 
	INT     21H
	JMP     MAIN
